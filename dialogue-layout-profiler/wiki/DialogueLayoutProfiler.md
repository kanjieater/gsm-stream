...
- classified dialogue/name/UI/non-dialogue regions with bbox coordinates
...
`--debug-log` appends replayable request/response entries through a buffered
writer:

```json
{"request":{"type":"observe_frame","frame":{}},"response":{"type":"prediction","prediction":{}}}
```

This file is for analysis and fixture generation. It is intentionally separate
from the durable learned profile. By default it flushes every 100 entries and on
explicit `flush`, `shutdown`, or process exit.
...
the target profile path. The profile has a dirty flag, so explicit or shutdown
flushes do not rewrite the profile if no observations changed it.
...
- returns explicit classified region groups for visualization and integration
